  ...
  const handleAPIRequest = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
      const projectDir = prompt.directory.includes("https://github.com") ? prompt.directory : path.resolve(process.cwd(), prompt.directory);
      const fullPrompt = await getPromptWithFiles(prompt, template, part, projectDir);
